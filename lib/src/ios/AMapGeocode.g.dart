...
class AMapGeocode extends AMapSearchObject with NSCoding, NSCopying {
  ...
  static Future<AMapGeocode> create__({ bool init = true /* ios only */ }) async {
    final __result__ = await kAmapSearchFluttifyChannel.invokeMethod<Ref>('ObjectFactory::createAMapGeocode', {'init': init});
    final object = AMapGeocode()..refId = __result__.refId;
    return object;
  }
  ...
  static Future<List<AMapGeocode>> create_batch__(int length, { bool init = true /* ios only */ }) async {
    ...
    final resultBatch = await kAmapSearchFluttifyChannel.invokeListMethod<Ref>('ObjectFactory::create_batchAMapGeocode', {'length': length, 'init': init});
    return resultBatch.map((it) => AMapGeocode()..refId = it.refId).toList();
  }
